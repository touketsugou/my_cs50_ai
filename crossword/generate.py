...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        chars = set(
            w[overlaps[1]] for w in self.domains[y]
        )

        tmp_words = set()

        for w in self.domains[x]:
            if w[overlaps[0]] in chars:
                tmp_words.add(w)
            else:
                modified = True
        
        if modified:
            self.domains[x] = tmp_words
    ...
    def ac3(self, arcs=None):
        ...
        if arcs is None:
            arcs = list()
            for i in range(0, len(self.domains)-1):
                for j in range(0, len(self.domains)-1):
                    if i != j:
                        arcs.append((list(self.domains.keys())[i], list(self.domains.keys())[j]))
    ...
    def assignment_complete(self, assignment):
        ...
        """
        return all(var in assignment and assignment[var] for var in self.crossword.variables)
    ...
    def consistent(self, assignment):
        ...
        for i in range(0, len(assignment)-2):
            for j in range(i+1, len(assignment)-1):
                var_i = keys[i]
                var_j = keys[j]
                word_i = assignment[var_i]
                word_j = assignment[var_j]
                overlap = self.crossword.overlaps[var_i, var_j]
                if overlap is not None:
                    if word_i[overlap[0]] != word_j[overlap[1]]:
                        return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        def count_ruled_out_options(value):
            ruled_out = 0
        ...
        return sorted(self.domains[var], key=count_ruled_out_options)
